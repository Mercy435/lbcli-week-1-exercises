...
# WRITE YOUR SOLUTION BELOW:
P2SH_VALID=$(bitcoin-cli -regtest -rpcwallet=treasurewallet validateaddress $P2SH_ADDR | jq -r '.isvalid')
...
INTERNAL_KEY=$(echo "$ADDR_INFO" | jq -r '.desc' | sed -E 's/^tr\(\[[^]]+\]([0-9a-f]{64})\).*/\1/')
...
echo "Derived  address: $DERIVED_ADDR_RAW"
